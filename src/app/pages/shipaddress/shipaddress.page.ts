...
@Component({
  ...
  templateUrl: "./shipaddress.page.html",
  styleUrls: ["./shipaddress.page.scss"],
})
export class ShipaddressPage implements OnInit, OnDestroy {
  ...
  async onSubmit() {
    ...
    this.loading.present();
    let serv = (await this.userServices.updateShippingAddress(this.obs)).subscribe((res) => {
      serv.unsubscribe();
      this.loading.dismiss();
      if (res[0].Result_Code == 1) {
        // this.toastService.show("Updated successfully.");
        Swal.fire('Successful',
        "Shipping address has been successfully updated.",
         "success");
