...
@Component({
  selector: "app-rptcustomisepackaging",
  templateUrl: "./rptcustomisepackaging.page.html",
  styleUrls: ["./rptcustomisepackaging.page.scss"],
})
export class RptcustomisepackagingPage implements OnInit, OnDestroy {
  ...
  async refreshmenu(){
    ...
    let idx = tempMenu.findIndex((x) => x.idx === 8);
  ...
  async download(){
    ...
    this.loading.present();
    let serv = (await this.orderService.downloadcustomisepackaging(obj)).subscribe((res) => {
      debugger;
      serv.unsubscribe();
      this.loading.dismiss();
      if (res[0].Result_Code == 1) {
