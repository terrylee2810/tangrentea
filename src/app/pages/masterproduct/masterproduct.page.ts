...
@Component({
  ...
  templateUrl: "./masterproduct.page.html",
  styleUrls: ["./masterproduct.page.scss"],
})
export class MasterproductPage implements OnInit, OnDestroy {
  ...
  async onSubmit() {
    ...
    let serv = (await this.userServices.addUser(this.obs)).subscribe((res) => {
      ...
      if (res[0].Result_Code == 1) {
        this.alertService
        .showOkay("You're almost there! We've sent a verification email to the address you provided. Didn't receive the email? Please check if the email is in your junk/spam folder.");
