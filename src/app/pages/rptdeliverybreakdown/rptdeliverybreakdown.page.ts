...
@Component({
  selector: "app-rptdeliverybreakdown",
  templateUrl: "./rptdeliverybreakdown.page.html",
  styleUrls: ["./rptdeliverybreakdown.page.scss"],
})
export class RptdeliverybreakdownPage implements OnInit, OnDestroy {
  ...
  async refreshmenu(){
    ...
    let idx = tempMenu.findIndex((x) => x.idx === 14);
  ...
  async download(){
    ...
    this.loading.present();
    let serv = (await this.orderService.downloaddeliverybreakdown(obj)).subscribe((res) => {
      debugger;
      serv.unsubscribe();
      this.loading.dismiss();
      if (res[0].Result_Code == 1) {
