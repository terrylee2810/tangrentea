...
@Component({
  ...
  templateUrl: "./memberprofile.page.html",
  styleUrls: ["./memberprofile.page.scss"],
})
export class MemberprofilePage implements OnInit, OnDestroy {
  ...
  public async ngOnInit(): Promise<void> {
    this.subscription = this.router.events.subscribe((event) => {
      if (
        ...
        event.url.includes("/memberprofile")
      ) {
      }
    });
  }
  ...
  async refreshmenu(){
    ...
    let idx = tempMenu.findIndex((x) => x.idx === 2);
  ...
  async onSubmit() {
    ...
    this.loading.present();
    let serv = (await this.userServices.updateUser(this.obs)).subscribe((res) => {
      serv.unsubscribe();
      this.loading.dismiss();
      if (res[0].Result_Code == 1) {
